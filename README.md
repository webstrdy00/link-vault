...
- 일반 실행은 `내 보관함`에서 시작합니다. 하단의 보관함·검색·설정으로 이동하며,
  별도 `+` 버튼(접근성 이름 `링크 저장`)으로 직접 입력합니다.
  저장은 탐색 탭과 구분하며, 외부 공유는 입력 확인 화면으로 바로 열립니다.
- 복구한 임시 입력은 첫 화면을 가리지 않고 `이어쓰기`로 열 수 있습니다.
  이미지 1장 공유의 기기 내 복사와 대상 항목에 대한 명시적 첨부 확인은 별개로 유지합니다.
- 얇은 구분선으로 나눈 목록 행 전체를 눌러 상세를 엽니다. 제목·출처·메모·분류·첨부 여부를 표시하며
  목록에 없는 썸네일을 만들거나 항목마다 이미지를 추가 조회하지 않습니다.
- 검색 조건은 `필터`에서 고르고 `필터 적용`으로 반영합니다. 적용 전 변경은 결과와
  구분해서 표시합니다. 분류 생성·수정은 `분류 관리`에서 접근합니다.
- 상세의 `더보기`에서 제목·메모 수정과 삭제를 선택합니다. 처리 정보와 분류 설정은
  펼쳐보되, 실패·충돌·미전송 상태 및 삭제 확인은 숨기지 않습니다.
- 공통 라이트·다크 테마와 읽기 중심 글자·간격·아이콘을 사용합니다.
  인증·검색 규칙·저장 큐·DB 스키마는 UI 개편 대상이 아닙니다.
- 추가 시각 검토는 Macapp Supply의 Letterboxx·Supaste·Hejour와 loadmo.re의
  In Common With 개별 화면을 비교했습니다. 자료 중심의 정보 밀도와 얇은 구획을
  참고하며, 홍보용 이미지·3D 효과·AI 기능을 제품 기능처럼 가져오지 않습니다.
- Pretendard 1.3.9 Regular/SemiBold/Bold를 앱에 포함해 한국어와 영문 서체를 통일합니다.
  실행 중 글꼴을 내려받지 않습니다. 원본 OTF의 Git blob 해시를 대조했으며
  `android/app/src/main/assets/licenses/pretendard-OFL.txt`에 원문 OFL 라이선스를 포함합니다.
  설정의 `글꼴 라이선스`에서 네트워크 연결 없이 읽을 수 있습니다.

2차 시각 개편 검증: JVM 98개, 별도 QA AVD의 전체 계측 시험 61개,
다크 모드·글자 배율 1.5의 공유/탐색/목록/필터 시험 14개가 통과했습니다.
Lint 오류 0개·기존 경고 48개이며, 최종 APK의 글꼴 3개와 라이선스 포함을 확인했습니다.
실제 회원 DB를 변경하지 않고 기존 로그인 AVD에 설치하여 보관함·검색·필터·설정과
로그인 유지를 확인했습니다. 많은 분류에서도 필터 적용 버튼이 보이는지, 잘못된 날짜가
시트를 닫지 않는지, 글꼴 라이선스가 열리고 닫히는지도 계측 시험으로 확인했습니다.
자료가 있는 화면은 별도 QA 기기의 합성 자료로 실제 Compose 컴포넌트를 렌더링했습니다.
`android/app/build/design-v2/`의 `populated.png`, `filters.png`는 해당 시험 이미지이고,
`live-library.png`, `live-search.png`, `live-filter.png`는 기존 로그인 앱 화면입니다.
첫 회차의 System UI 응답 없음으로 발생한 초점 실패 2개는 통과로 집계하지 않았으며,
외부 대화상자를 확인·정리한 뒤 단언 변경 없이 전체 시험을 다시 통과했습니다.
빈 DB 전용 M1–M5 서버 통합 runner는 회원 자료 보존을 위해 재실행하지 않았고,
바뀐 UI 계측 소스 전체의 최종 컴파일은 통과했습니다. 커밋·푸시는 하지 않았습니다.

1차 UI 개편 검증: JVM 98개, 별도 `LinkVault_DesignQA` AVD의 전체 계측 시험 58개,
다크 모드·글자 배율 1.5의 공유/탐색/카드 시험 11개가 통과했습니다.
Lint는 오류 0개·기존 경고 48개이고 Google 설정을 포함한 debug APK를 빌드했습니다.
검증 중 발견한 회전 후 검색 탭 초기화, 상세 뒤로가기, 다크 상태표시줄 대비를 수정했습니다.
System UI 응답 없음으로 초점을 잃은 회차와 Kotlin FIR 내부 Lint 오류 회차는
통과로 집계하지 않았으며, 대화상자 확인·정리 및 새 Gradle 프로세스에서 다시 검증했습니다.
기존 로그인 AVD에는 앱 데이터 삭제 없이 APK만 갱신했고 Google 로그인 유지와
보관함·검색·필터·설정 접근을 확인했습니다.
실제 회원이 있는 DB를 보존하기 위해 빈 환경 전용 M1–M5 통합 runner는 다시 실행하지
않았고, 변경된 해당 계측 소스는 최종 코드와 함께 컴파일했습니다.
이 검증은 실제 SNS·새로운 Google 재인증·삭제/탈퇴 실사용 인수나 외부 베타 승인이 아닙니다.

## M6 베타 준비와 실제 인수

2026-09-20 M6 준비 작업으로 테스트 APK와 실제 수집 CSV 검증 도구를 마련했습니다.
**외부 베타 시작 승인은 아직 아닙니다.** 실제 Google 인증·탈퇴 재인증, S23의 SNS
공유, 본인 자료 30건·재발견 과제 10개, OCR·분류 품질과 운영 인수가 남아 있습니다.
합성 단위 테스트 자료를 실제 사용자 자료로 채워 넣지 않습니다.

### 테스트 빌드와 사용자 안내

위 `assembleDebug` 결과는 본인 테스트용 debug APK입니다. 빈 인증 설정으로도 빌드되므로
빌드 성공을 로그인 가능 또는 배포 준비 완료로 해석하지 않습니다. 실제 계정 사용 전에는
아래 Google 설정 절차를 완료하고, S23에서는 접근 가능한 HTTPS backend를 사용하세요.
`10.0.2.2`는 에뮬레이터 전용이며 S23의 서버 주소가 아닙니다.
배포 서명·외부 배포 채널은 아직 구성하지 않았습니다.

1. Google 로그인 후 승인된 계정에서 시작합니다. 승인 대기는 오류가 아닙니다.
2. 실제 SNS에서 공유하고 URL 후보를 확인해 저장합니다. 메모에 기억할 단서를 남깁니다.
3. 이미지는 저장된 항목을 명시적으로 선택한 뒤 첨부합니다. OCR 실패와 이미지 저장 실패는
   별개이며, 원문 사이트의 로그인 요구·삭제도 앱 저장 실패와 구별합니다.
4. 오프라인 저장은 대기 상태를 확인하고 재연결 후 실제 저장 결과를 확인합니다.
5. 검색은 키워드·정해진 별칭 기준입니다. 자연어 이해나 영상 분석을 제공하지 않습니다.
6. 삭제·탈퇴는 되돌릴 수 없습니다. 개인 자료 없는 전용 시험 계정에서 검증합니다.
   오류 제보에는 재현 순서·빌드 버전·고정 오류 코드만 적고 URL·메모·토큰은 보내지 않습니다.

### 30건·10과제 기록 검증

기존 `docs/관심 정보 통합 보관함/검증자료/device_capture_template.csv`와
`retrieval_tasks_template.csv`를 **저장소 밖의 개인 보관 경로**에 복사해 기록합니다.
원문 URL·검색어가 들어 있으므로 CSV 원본을 커밋하거나 자동 전송하지 않습니다.
첫 행의 열 이름과 순서를 유지하며 UTF-8 CSV의 따옴표·쉼표를 지원합니다.

- 출처 `source`는 정확히 `Threads` 8건, `Instagram` 8건, `네이버 블로그` 8건,
  `기타 공개 웹` 6건입니다. `case_id`와 `task_id`는 각 파일에서 중복되지 않아야 합니다.
- 수집 행의 `app_version`, `android_version`, `mime`, `memory_cue`는 필수입니다.
  `save_status`는 `saved` 또는 `input_error`이며 후자는 `failure_reason`을 기록합니다.
  boolean 값은 `true`/`false`, 시간은 음수가 아닌 숫자로 씁니다.
- 기억 단서와 기대 원문을 시험 전에 정합니다. 과제의 `case_id`는 수집 행을 참조하고
  `memory_cue`는 미리 적은 값과 같아야 합니다. `expected_item_id`는 기대 항목 ID입니다.
- `route`는 `search`/`category`, `success`는 `true`/`false`,
  `seconds_to_original`은 원문 도달 시간입니다. 실패 원인도 기록합니다.
  검색 과제는 `first_query`와 `search_hit_at_5`(`true`/`false`)가 필수입니다.
  `tested_at`은 `2026-09-20T15:00:00+09:00`처럼 시간대를 포함합니다.
- 정확히 10개 과제 중 60초 이내 성공 8개 이상을 AT07 목표로 집계합니다.
  AT01의 명시적 입력 오류와 실제 저장 30건 달성은 별도 집계입니다.

```powershell
npm run test:beta
npm run beta:check -- --captures C:\private\captures.csv --tasks C:\private\tasks.csv
```

도구는 파일당 1MiB·100행·셀당 8KiB를 제한하고 원문 대신 집계와 고정 코드만 출력합니다.
빈 양식·누락·중복·잘못된 참조·모순된 성공 기록·목표 미달은 exit 1입니다.
exit 0도 **입력한 기록의 AT01·07·08 집계만** 뜻합니다. 기록의 진실성·사전 작성 여부는
CSV로 입증할 수 없어 `real_user_evidence_verified`와 `external_beta_ready`는 항상 false입니다.
실제 Google/S23·OCR 10장 중 8장·분류 수용률 80%·운영 및 보안 인수는 별도 확인합니다.

현재 검증: 집계 도구 17개, 운영 도구 회귀 24개, JVM·Lint·debug 빌드와 에뮬레이터
54개가 통과했습니다. 초기 에뮬레이터 부팅 및 System UI 응답 없음으로 실패한 회차는
제외했고, 해당 대화상자를 정리한 뒤 전체 54개를 다시 통과했습니다.
기존 실제 기록 양식은 비어 있어 `CAPTURES_CSV_HEADER_ONLY`로 거부되는 것을 확인했습니다.
실제 사용자 인수 및 외부 베타 개시는 미완료입니다.

### S23 없이 에뮬레이터로 진행한 추가 인수 (2026-09-20)

사용자 요청에 따라 Android Studio AVD와 실제 로컬 Supabase로 M1~M5 연동을
다시 실행했습니다. 저장·재조회, 오프라인/강제 종료/재연결, 검색·분류·재적용,
공유 이미지·실제 한글/영문 OCR·교체·삭제, 계정 삭제 접수 후 개인 자료 정리가 통과했습니다.
API 200개, 실제 검색 80개·enrichment 69개·삭제 38개·동시성 3개,
격리 백업/복원 12개와 CSV 검증 17개도 재통과했습니다.
이는 에뮬레이터 기능 인수이며 S23 및 실제 SNS 앱 조합 호환성 인수는 아닙니다.

초기 Edge container가 종료돼 발생한 연결 실패는 복구 후 재검증했습니다.
이후 `backend:start`도 CLI exit 0만 믿지 않고 실제 Edge `/v1/health`의
DB 준비 상태까지 확인하도록 수정했습니다. 제한된 재시도 후에도 준비되지 않으면
`LOCAL_BACKEND_HEALTH_UNAVAILABLE`로 실패하며 원문 응답·인증 키는 출력하지 않습니다.
`npm run test:backend-start`의 6개 시험과 실제 시작·Auth/API/RLS 42개를 통과했습니다.
탈퇴 재인증 시험의 BACK 입력이 앱 화면을 닫는 경합은 시험 코드에서
실제 Google/권한 제공자 창이 보일 때만 취소하도록 수정했습니다.
별도 재시도에서 초기 session 변경으로 실패한 회차도 있었으며 성공으로 집계하지 않았습니다.
후속 전체 삭제 시험은 통과했지만 이 결과를 반복 실행 안정성 보증으로 확대하지 않습니다.

후속 사용자 승인으로 별도 Google Cloud `link-vault-509214` 프로젝트를 생성하고
웹 audience·Android debug OAuth 클라이언트를 구성했습니다. 기존 프로젝트는 변경하지
않았고 결제 연결·유료 리소스·앱 공개 게시도 하지 않았습니다. 외부 테스트 모드와
본인 테스트 사용자 1명을 확인했습니다. Android 등록은 `com.linkvault.app`과 이 개발
기기의 debug 서명에 한정되며 release 서명 등록을 대신하지 않습니다.
로컬 Google provider 활성화와 실제 설정을 넣은 APK 빌드는 확인했습니다.
사용자가 에뮬레이터 Google 로그인을 마친 뒤 Credential Manager에서 실제 계정을
선택하고 동의해 Supabase Google 토큰 교환을 완료했습니다. 앱의 승인 대기 상태,
로컬 회원 승인 후 `로그인했어요`, 앱 프로세스 강제 종료·재시작 뒤 로그인 복원을
직접 확인했습니다. 서버에서도 실제 Google identity·승인 회원·active profile이
각각 1개임을 원문 없는 집계로 검증했습니다.
이 결과는 이메일 fixture 대체가 아닌 실제 Google 로그인 증거입니다.
토큰 만료 후 갱신·실제 탈퇴 재인증·운영 환경 인증까지 검증한 것은 아닙니다.
현재 로컬 DB에는 실제 Google 테스트 회원이 있으므로 빈 환경 전용 통합 runner를
실행하려고 이 계정이나 DB를 삭제하지 않습니다. 계정과 로그인 상태를 보존했습니다.
Google 로그인 자체는 Google Play 지원 AVD에서도 검증할 수 있지만 실제 설정과 사용자
로그인·동의가 필요합니다. 이메일 fixture 로그인이나 암호 서명 단위 시험으로 대체 인증하지 않습니다.
실제 SNS 공유는 해당 앱 설치·사용자 로그인·공유 조작이 있어야 하며, 합성 공유 intent와
구별합니다. 본인 30건·10과제의 기억/시간 측정, OCR 10장의 핵심 단서 판정과
분류 수용 여부는 사용자 평가가 필요합니다. 이 과정에 S23이 반드시 필요한 것은 아닙니다.

운영 진단에서 정리 대기·초과·사용량 불일치·미추적 객체는 모두 0이고 cron은 동작했습니다.
영속 운영 원장 보관 미설정인 `DELETION_LEDGER_NOT_EXPORTED` 경고(exit 1)는 유지됩니다.
격리 복원 시험 성공을 실제 운영 archive·키 보관·경보·운영 복구 인수로 대신하지 않습니다.
...
- 서버의 Google provider에는 같은 웹 client ID와 필요한 client secret을 설정합니다.
  ...
  현재 로컬 provider는 활성화돼 있으므로 새 개발 환경에서도 이 값이 필요합니다.
  Supabase용 값은 gitignored 루트 `.env`, Edge의 audience는
  `supabase/functions/.env`에 보관하며 키·토큰을 출력하거나 커밋하지 않습니다.
  Gradle은 루트 `.env`를 자동으로 읽지 않으므로 앱 빌드에는 위 설정을 별도로 전달합니다.
  nonce 검증은 끄지 않습니다.
