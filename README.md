...
Android 우선 관심 정보 보관함. M0 공유 입력과 M1 회원·서버 저장·목록·상세를 구현했습니다.
Google 로그인 코드는 연결했지만 실제 OAuth 공급자 교환은 설정 후 검증이 필요합니다.
검색·자동 분석·첨부·오프라인 대기열·삭제/탈퇴는 아직 제공하지 않습니다.

## 개발 환경

- JDK 17, Android SDK Platform 35, minSdk 26
- Gradle 8.10.2 Wrapper, AGP 8.7.3, Kotlin 2.0.21
...
마지막 명령은 실행 중인 Android 에뮬레이터를 선택합니다.
...
DB pgTAP 125개, API 계약 22개, 실제 로컬 Auth/API/RLS 통합 32개를 통과했습니다.
Android JVM 26개·기본 에뮬레이터 UI 9개·실제 로컬 서버 연결 UI 1개를 통과했습니다.
Android Lint는 오류 0개·경고 31개입니다. 버전 업데이트 안내 30개와
`CredentialManagerSignInWithGoogle` 정적 경고 1개를 숨기지 않고 유지했습니다.
...
M1의 실제 Google 로그인 인수는 미완료이며 M2 완료나 전체 베타 완료를 뜻하지 않습니다.
